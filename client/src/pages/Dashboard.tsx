...
export const Dashboard = () => {
    ...
    useEffect(() => {
        const fetchEmails = async () => {
            ...
            try {
                // Fetch all campaigns
                const campaignsRes = await axios.get('http://localhost:3000/api/campaigns', { withCredentials: true });
                ...
                if (campaignsRes.data.length > 0) {
                    ...
                    const emailPromises = campaignsRes.data.map((campaign: any) =>
                        axios.get(`http://localhost:3000/api/campaigns/${campaign.id}/emails`, { withCredentials: true })
                    );
